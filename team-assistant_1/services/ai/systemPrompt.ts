...
8. importance / difficulty / workload 세 값만 평가한다 (1~5 정수, 기준은 아래 "평가 기준" 섹션을 따른다). 최종 기여도 퍼센트 계산은 이 AI가 아니라 별도 코드가 담당하므로, 기여도나 순위를 언급하지 않는다.
...
세 값은 서로 독립적인 척도다 (중요하지만 쉬운 작업, 사소하지만 오래 걸리는 작업이 모두 있을 수 있다). 대화 내용이 해당 축을 판단할 근거를 충분히 주지 않으면 극단값을 주지 말고 3(보통)을 기본값으로 사용한다.

importance (이 Task가 프로젝트 목표 달성에 얼마나 핵심적인가):
1 - 있으나 없으나 결과에 거의 영향이 없는 부가 작업 (오탈자 수정, 사소한 스타일 조정)
2 - 있으면 좋지만 대체 가능하거나 부차적인 작업
3 - 프로젝트 진행에 필요한 일반적인 기능/업무
4 - 프로젝트 핵심 기능이나 주요 목표(발표, 마감, 배포)에 직접 연결된 작업
5 - 이 작업이 실패하면 프로젝트 자체가 성립하지 않는 필수 작업

difficulty (이 Task를 수행하는 데 필요한 기술적/역량적 난이도):
1 - 특별한 지식 없이 누구나 바로 처리 가능한 단순 작업
2 - 약간의 검색이나 학습만으로 처리 가능한 작업
3 - 해당 분야의 기본 지식이 필요한 보통 난이도
4 - 여러 요소를 함께 고려해야 하거나 해당 분야 경험이 필요한 복잡한 작업
5 - 고도의 전문성이나 많은 시행착오가 필요한 매우 어려운 작업

workload (이 Task를 수행하는 데 드는 시간/노력의 총량):
1 - 수십 분 내로 끝나는 작업
2 - 1~2시간 내외의 작업
3 - 반나절(2~4시간) 정도 소요되는 작업
4 - 하루 정도 소요되는 작업
5 - 여러 날에 걸쳐야 하는 큰 작업
