...
export default function TaskDetailPage({
  ...
  return (
    <div className="page-container max-w-5xl">
      <div className="mb-7"><p className="page-eyebrow">TASK</p><h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-950">업무 상세</h1><p className="mt-1.5 text-sm text-slate-500">업무 정보와 평가, 기록 근거를 확인하고 수정하세요.</p></div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
            ...
            <div className="mt-5 rounded-xl bg-slate-50 p-4">
              <div className="grid gap-2 sm:grid-cols-2">
                {members.map((member) => {
                  ...
                  return <label key={member.id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 ${selected ? "border-violet-200 bg-violet-50" : "border-slate-200 bg-white"}`}><input type="checkbox" checked={selected} onChange={() => toggleContributor(member.id)} disabled={saving} className="accent-[#6541f3]" /><span className="min-w-0 flex-1 truncate text-sm text-slate-700">{member.avatarEmoji} {member.name}</span>{selected && <input type="number" min={1} max={100} step={1} value={contributorDrafts[member.id]} onChange={(event) => setContributorDrafts((current) => ({ ...current, [member.id]: event.target.value }))} onClick={(event) => event.stopPropagation()} disabled={saving} aria-label={`${member.name} 참여 비율`} className="glass-input w-16 rounded-lg px-2 py-1 text-right text-sm" />}{selected && <span className="text-xs text-slate-400">%</span>}</label>;
                })}
              </div>
        ...
        <div className="mt-7 border-t border-slate-200 pt-5"><p className="mb-3 text-xs font-bold text-slate-500">업무 평가</p><div className="grid grid-cols-3 divide-x divide-slate-200 rounded-xl bg-slate-50 text-center">
        ...
        {task.lastReason && (
          <p className="mt-4 rounded-xl bg-violet-50 px-3 py-3 text-xs leading-5 text-slate-600">
            분석 판단 근거: {task.lastReason}
          </p>
        )}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        <div className="mb-4"><p className="page-eyebrow">EVIDENCE</p><h2 className="mt-1.5 text-base font-bold text-slate-900">기록 근거 <span className="text-[#6541f3]">{evidence.length}</span></h2></div>
        {evidence.length === 0 ? (
          <p className="text-sm text-slate-500">아직 근거가 없습니다.</p>
        ) : (
          <ul className="relative flex flex-col gap-4 before:absolute before:bottom-4 before:left-[7px] before:top-4 before:w-px before:bg-slate-200">
            {evidence.map((ev) => (
              <li key={ev.id} className="relative ml-5 border-b border-slate-100 pb-4 text-sm text-slate-700 last:border-b-0 last:pb-0 before:absolute before:-left-[21px] before:top-1 before:h-3 before:w-3 before:rounded-full before:border-2 before:border-white before:bg-[#6541f3]">
...
function ScoreBox({ label, value }: { label: string; value: number }) {
  return (
    <div className="py-3">
      ...
      <p className="mt-0.5 text-lg font-bold text-[#6541f3]">{value}<span className="text-xs font-medium text-slate-400">/5</span></p>
    </div>
  );
}
