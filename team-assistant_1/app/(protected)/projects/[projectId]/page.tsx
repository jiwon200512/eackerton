      ...
      <section className="glass-card rounded-2xl p-4 sm:p-5">
        ...
        {visibleTasks.length === 0 ? (
          ...
          <ul className="mt-3 flex flex-col gap-2">
            {visibleTasks.map((t) => <li key={t.id}><button onClick={() => router.push(`/projects/${projectId}/tasks/${t.id}`)} className="group w-full rounded-xl border border-white/70 bg-white/48 p-4 text-left hover:border-indigo-200 hover:bg-white/75"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-slate-800 group-hover:text-indigo-700">{t.title}</p><div className="mt-2"><TaskContributors contributors={t.contributors} compact /></div></div><StatusBadge status={t.status} /></div><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-slate-400"><span>중요도 <b className="text-slate-600">{t.importance}</b></span><span>난이도 <b className="text-slate-600">{t.difficulty}</b></span><span>작업량 <b className="text-slate-600">{t.workload}</b></span></div></button></li>)}
          </ul>
        )}
      </section>
